<template>
  <div>
      <h2 class="title">bbs主题列表</h2>
      <Button type="success" size="large" icon="plus" @click="add">新增主题</Button>
...
export default {
  ...
  methods:{
    add(){
      this.$router.push({name:'bbs-theme-add'});
    },
    edit(index){
      this.$router.push({name:'bbs-theme-edit',params: { theme_id: this.data[index].id }})
    },
    // 页码切换
    pageChange(p) {
      this.p = p;
    ...
    loading() {
      this.spinShow=true;
      this.$socket("bbs@/subject/index",{ p: this.p },res => {
